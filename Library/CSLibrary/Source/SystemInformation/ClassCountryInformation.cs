...
namespace CSLibrary
{
    ...
    public static class DEVICE
    {
        class COUNTRY
        {
            ...
            public RegionCode defaultFrequency;

            public COUNTRY (int countryCode, int specialCountryCode, string countryName, RegionCode defaultFrequency)
            {
        ...
        {
            return (countryCode.ToString() + " " + (country.Find(item => item.countryCode == countryCode && item.specialCountryCode == specialCountryCode)).countryName);
        }

        public static RegionCode GetDefauleFrequency(int countryCode, int specialCountryCode)
        {
            return ((country.Find(item => item.countryCode == countryCode && item.specialCountryCode == specialCountryCode)).defaultFrequency);
        }
    }
}
