...
{
    public class ViewModelUCODE8Inventory : BaseViewModel
	{
        ...
        public ViewModelUCODE8Inventory(IAdapter adapter, IUserDialogs userDialogs) : base(adapter)
        {
        ...
        public override void ViewDisappearing()
        {
            ...
            //BleMvxApplication._reader.rfid.SetBrandID(false);
            SetEvent(false);
        ...
        void SetConfigPower()
        {
            ...
            {
                for (int cnt = BleMvxApplication._reader.rfid.GetAntennaPort() - 1; cnt >= 0; cnt--)
                {
        ...
        void InventorySetting()
        {
            ...
            // Set UCODE 8 BandID 
            {
                //BleMvxApplication._reader.rfid.SetBrandID(true);
                BleMvxApplication._reader.rfid.Options.TagSelected.flags = CSLibrary.Constants.SelectMaskFlags.ENABLE_TOGGLE;
                BleMvxApplication._reader.rfid.Options.TagSelected.bank = CSLibrary.Constants.MemoryBank.EPC;
                BleMvxApplication._reader.rfid.Options.TagSelected.epcMask = new CSLibrary.Structures.S_MASK("80");
                BleMvxApplication._reader.rfid.Options.TagSelected.epcMaskOffset = 0x204 - 0x20; // Fix library auto offset feature
                BleMvxApplication._reader.rfid.Options.TagSelected.epcMaskLength = 1;
...

                {
                    ...
                    extraSlecetion.mask = new CSLibrary.Structures.SelectMask(CSLibrary.Constants.MemoryBank.TID, 0x0, 32, new byte[] { 0xE2, 0x80, 0x68, 0x94 });
...
//                    if (epcstr.Substring(epcstr.Length - 4, 4) != "AAAA")
//                        return;

                    for (cnt = 0; cnt < TagInfoList.Count; cnt++)
                    {
                        if (TagInfoList[cnt].EPC == epcstr)
                        {
